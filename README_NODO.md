...
| 5 | `CustomerVerifiedBadge.vue` (new) + `ContactInfo.vue` (edit) + `components-next/Conversation/ConversationCard/ConversationCard.vue` (edit 5b) | **Insignia ✓ azul "Cliente verificado"** al lado del nombre del contacto cuando `custom_attributes.etapa_comercial === 'Cierre'`. Aparece en (a) panel "Información de contacto" y (b) lista de conversaciones (bandeja de entrada) — para visualización rápida de qué contactos ya son clientes mientras el agent trabaja la bandeja. Acompañado de 7 Custom Attributes nativos creados en la cuenta (ver sección "Tab Comercial — Nivel B" más abajo) que conforman un mini-CRM dentro de Chatwoot **sin necesidad de tablas/UI custom**. Estética tipo Twitter/X verified. |
