...
class Llm::SpeechToTextService < Llm::LegacyBaseOpenAiService
  ...
  # NODO PATCH 11: el endpoint de transcripción de OpenAI valida por EXTENSIÓN y
  # rechaza las variantes poco comunes con 400 (que el job descarta en silencio).
  # Mismo contenedor/codec con el nombre "aceptado": las notas de voz de Evolution
  # llegan como audio/opus y Rails las nombra .oga → 400; renombradas .ogg
  # transcriben perfecto (igual que las de WhatsApp Cloud).
  ...
  def fetch_audio_file
    ...
    # NODO PATCH 11: upstream solo deriva la extensión cuando el filename no trae
    # ninguna, así que un .oga/.opus/.mpga llega intacto a OpenAI y da 400. Hay
    # que resolverla SIEMPRE (del filename o del content_type) y normalizarla.
